...
contract TakeProfitsHook is BaseHook, ERC1155 {
    // Use the PoolIdLibrary for PoolKey to add teh `.toId()` function to the PoolKey
    // which hashes teh PoolKey struct inot a bytes32 value
    using PoolIdLibrary for PoolKey;

    // Use CurrencyLibrary for the Currency Struct
    using CurrencyLibrary for Currency;
    using FixedPointMathLib for uint256;

    // mapping to store the last know tickLower for a given pool
    mapping(PoolId poolId => int24 tickLower) public tickLowerLasts;

    // A nested mapping to store teh take-profit order placed by users
    // mapping is PoolId => tickLower => zeroForOne => amount
    ...
    // tokenIdTotalSupply is a mapping that stores how many tokens need to be sold to execute teh take-profit order
    ...
    // Initialze the BaseHook and ERC1155 parent contracts with the constructor
    constructor(IPoolManager _poolManager, string memory _uri) BaseHook(_poolManager) ERC1155(_uri) {}

    // Require override function for BaseHook to let the PoolManager know which hooks are implemented
        ...
        returns (bytes4)
    {
        ...
        emit DebugBytes(abi.encodePacked("Pool ID: ", key.toId())); // Debug the Pool ID
        emit DebugInt(tick);  // Debug the tick
        emit DebugInt(key.tickSpacing); // Debug the tickSpacing

        emit DebugString("Entering afterInitialize");
        _setTickLowerLast(key.toId(), _getTickLower(tick, key.tickSpacing));
        emit DebugString("Exiting afterInitialize");
    ...
    function afterSwap(address, PoolKey calldata key, IPoolManager.SwapParams calldata params, BalanceDelta, bytes calldata)
        ...
        returns (bytes4)
    {
        emit DebugString("Entering afterSwap");
        int24 lastTickLower = tickLowerLasts[key.toId()];
        emit DebugInt(lastTickLower);
        // emit DebugString(abi.encodePacked(key.toId()));
        ...
        // i.e. if someone does a zerForOne swap to increase price of Token 1, we execute all orders that are oneForZero
        // and vice-versa
        ...
        emit DebugInt(currentTickLower);
        emit DebugString("Exiting afterSwap");
    ...
    function placeOrder(PoolKey calldata key, int24 tick, uint256 amountIn, bool zeroForOne) external returns (int24) {
        ...
        // if toke id doesn't exist add it to the mapping
        // Not every order creates a token id, it's possibble for users to ad more tokens to a pre-existing order
        ...
        emit DebugString("Exiting placeOrder");
        return tickLower;
    }
    ...
    function cancelOrder(PoolKey calldata key, int24 tick, bool zeroForOne) external {
        ...
        // Get teh amount of tokens the user's ERC-1155 tokens represent
        ...
        // Extract the address of the token tohe user wanted to sell
        address tokenToBeSoldContract = zeroForOne ? Currency.unwrap(key.currency0) : Currency.unwrap(key.currency1);

        // Move the tokens to be sold from mthis contract back to the user
    ...
    function fillOrder(PoolKey calldata key, int24 tick, bool zeroForOne, int256 amountIn) internal {
        ...
        emit DebugString("Entering fillOrder");
        IPoolManager.SwapParams memory swapParams = IPoolManager.SwapParams({
            zeroForOne: zeroForOne,
            amountSpecified: amountIn,
            // set the price limit to be the least possible if swapping from Token 0 to Token 1
            // or the maximum possible if swapped from Token 1 to Token 0
            // i.e. inifinite slippage allowed
        ...
        // Update the mapping to reflect that `amountIn` worth of tokens have been swapped for the order
        ...
        emit DebugString("Exiting fillOrder");
    }
    ...
    function redeem(uint256 tokenId, uint256 amountIn, address destination) external {
        ...
        // Make sure the user has enough ERC-1155 tokens to redeem the amount they're requesting
        uint256 balance = balanceOf(msg.sender, tokenId);
        require(balance >= amountIn, "TakeProfitsHook: Not enough ERC-1155 to redeem requested amount");
        ...
        // amountToSend = amount * (total claimable / total supply)
    ...
    // ERC-1155 helpers to track users orders
    ...
    function _getTickLower(int24 actualTick, int24 tickSpacing) private pure returns (int24) {
        // require(actualTick != 0, "Actual tick cannot be zero");
        int24 intervals = actualTick / tickSpacing;
        if (actualTick < 0 && actualTick % tickSpacing != 0) intervals--; // round towards negative infinity
        // emit DebugString("getTickLower starting at ");
        // emit DebugInt(tickSpacing);
        // emit DebugInt(actualTick);
